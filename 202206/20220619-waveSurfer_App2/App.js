...
export default function App() {
  ...
  useEffect(() => {
    ...
    waveformMediaEleRef.current.on("waveform-ready", function() {
  ...
  const onVolumeChange = e => {
    ...
    if (newVolume) {
      ...
      waveformMediaEleRef.current.setVolume(newVolume);
    }
  }
  ...
  function handlePitchToggle() {
    ...
    // 一旦、シーク時間を変数に格納
    let timeSeekRef = waveformRef.current.getCurrentTime();
    let timeDurRef = waveformRef.current.getDuration();
    let timeRatioRef = timeSeekRef / timeDurRef;

    // トグルを切り替えたタイミングで音声読み込み
    waveformMediaEleRef.current.load('https://k-tsuda400.github.io/files/20220305/test.mp3');
    // 読み込んだあとにseekToすれば再生時刻変更できる？ →　できた
    {
      waveformMediaEleRef.current.on("waveform-ready", function() {
        if (waveformMediaEleRef.current) {
          waveformMediaEleRef.current.setVolume(volume);
          setVolume(volume);
          // wavefor-readyイベントの中でseekToセットすればいける
          waveformMediaEleRef.current.seekTo(timeRatioRef);
          waveformMediaEleRef.current.playPause();
        }
      });
    }
